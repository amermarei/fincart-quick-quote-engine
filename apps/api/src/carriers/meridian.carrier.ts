...
/**
 * Meridian Freight pricing strategy.
 *
...
export class MeridianCarrierStrategy implements CarrierStrategy {
  ...
  async run(shipment: Shipment, ctx: MerchantContext): Promise<CarrierOutcome> {
    return this.price(shipment, ctx);
  }

  /**
   * Prices a shipment per the contract rate card. Public so the exactness
   * test suite can call it directly; `run` delegates here.
   */
  price(shipment: Shipment, ctx: MerchantContext): CarrierOutcome {
    ...
    const remote = this.isRemoteArea(shipment.destination.postcode)
  ...
  private isRemoteArea(postcode: string): boolean {
...
/**
 * Thin wrapper for the exactness test suites — identical code path to the
 * strategy's `price`, kept as a plain function for direct unit testing.
 */
export function priceMeridian(
  shipment: Shipment,
  ctx: MerchantContext,
): CarrierOutcome {
  return new MeridianCarrierStrategy().price(shipment, ctx);
}